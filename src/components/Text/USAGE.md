...
This document describes how to integrate and use the `Text` component in your projects using `@recursica/adapter-mantine-v8`.
...
```tsx
import { Text } from "@recursica/adapter-mantine-v8";
```
...
`Text` exposes two Recursica-native, token-bound props (not Mantine's freeform `color`):
