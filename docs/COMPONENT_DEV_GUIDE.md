...
This document covers what's specific to building components in `adapter-mantine-v8`. The full shared rulebook (styling rules, prop layer, folder structure, tokens, testing, checklist) lives in the canonical [`packages/adapter-common/docs/COMPONENT_DEV_GUIDE.md`](../../adapter-common/docs/COMPONENT_DEV_GUIDE.md) — read that first. This document only covers what's different for Mantine.
...
Because this package (`@recursica/adapter-mantine-v8`) is explicitly built for Mantine, we do not need a generic adapter abstraction. The component itself serves as both the public API and the Mantine implementation.
...
- **Spread `sanitizedProps` first, always, and resolve override intent by name — not by ordering** (see the canonical guide's §3.3). Ordering alone can't distinguish "our value always wins" from "the caller may override our default," so use `withCallerOverride` for computed defaults, and `mergeClassNames`/`mergeStyles` for `classNames`/`styles` slot props (Mantine's per-slot merge needs this even though unmatched slot keys are more forgiving than MUI's `classes` — a caller-supplied slot Recursica has no default for must still pass through). Several components in this adapter (e.g. `Radio.tsx`) currently spread last, or override `classNames`/`icon` by bare placement with no helper — treat both as the pattern to fix, not to copy, when you touch one of them.
